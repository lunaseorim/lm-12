...
summary(coxph(Surv(time,status==1)~`LN수(+)`+width+stage+Cell.type+ki.67a,data = datset))

f_m <- coxph(Surv(time,status==1)~`LN수(+)`+width+stage+Cell.type+ki.67a,data = datset)


summary(f_m)

ggforest(f_m,data = datset)
...
# model + rx
result_b <- coxph(Surv(time,status==1)~`LN수(+)`+width+stage, data = datset)
...
result_b <- coxph(Surv(time,status==1)~`LN수(+)`+stage+Cell.type+ki.67a, data = datset)
median_b <- median(predict(result_b, data=datset))
df_b = cbind(datset[,colnames(datset)=='time'], datset[,colnames(datset)=='status'], predict(result_b, data=datset))
colnames(df_b) <- c('Time','status', 'Predict')
df_b <- data.frame(df_b)
...
model_result_b <- summary(coxph(Surv(Time,status==1) ~ df_b$Predict_cox, data=df_b))
logrank_b <- model_result_b$logtest[3]
fit3 <- survfit(Surv(Time,status==1) ~ df_c$Predict_cox, data=df_a)
